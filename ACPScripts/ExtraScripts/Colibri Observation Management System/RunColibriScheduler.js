...
function transformAltAzToRadDec(requests) {
    for (var i = 0; i < requests.length; i++) {
        ...
        if (request.alt != null && request.az != null) {
            ...
            Console.PrintLine(request.alt);
            Console.PrintLine(request.az);
            try{
            ...
            Console.PrintLine("Before set");
            Console.PrintLine("Right assention" + ct.RightAscension)
            requests[i].ra = ct.RightAscension
            requests[i].dec = ct.Declination
            Console.PrintLine("After set");
        }
    }
...
if (logconsole == true) {
    ...
    Console.LogFile = "d:\\Logs\\SatelliteDetectionData\\" + Util.FormatVar(Util.SysUTCDate, "yyyymmdd_HhNnSs") + "-ACPconsole.log";
...
LogFile = "d:\\Logs\\SatelliteDetectionData\\" + JDtoUTC(sunset) + "-ACP.log"; // Log file path based on sunset time.
...
function main() {
    ...
    while (Weather.Available && !Weather.safe && !testing) {
        ...
        if (getDate() != currentDate) {
            ...
            LogFile = "d:\\Logs\\SatelliteDetectionData\\" + JDtoUTC(sunset) + "-ACP.log";
	...
	if (firstRun = true) {
        ...
        Util.ShellExec("cmd.exe", "/c mkdir -p d:\\SatelliteDetectionData\\" + today.toString() + "\\Dark\\")
        
        // Log the creation of the directory.
        updateLog("Created today's data directory at d:\\SatelliteDetectionData\\" + today.toString(), "INFO");
    ...
    do {
        ...
        Util.ShellExec("cmd.exe", "/c mkdir -p d:\\SatelliteDetectionData\\" + today.toString() + "\\" + bestObs.directoryName)
        Util.ShellExec("cmd.exe", "/c mkdir -p d:\\SatelliteDetectionData\\" + today.toString() + "\\Dark\\" + bestObs.directoryName)
        ...
        while (Util.SysJulianDate <= endJD) {
            ...
            if (darkCounter == darkInterval) {
                ...
                refCollection(2, bestObs.exposureTime, "D:\\SatelliteDetectionData\\" + today.toString() + "\\Dark\\" + bestObs.directoryName);
            ...
            try {
                ...
                // If the image is ready, save it to the specified file path.
                if (ccdCamera.ImageReady) {
                    var filePath = "D:\\SatelliteDetectionData\\" + today.toString() + "\\" + bestObs.directoryName + "\\image_" + new Date().getTime() + "_" + bestObs.exposureTime + "s.fits"; 
